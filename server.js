...
function localSplit(title){
  ...
  // Flow 1.3.8: локальный смысловой декомпозитор.
  // 1) Если пользователь уже перечислил несколько действий — выделяем их.
  // 2) Если задача сформулирована крупно ("подготовиться...", "организовать...",
  //    "убраться...") — создаём практические шаги по типу задачи.
  const verbs = [
    'дойти','пойти','сходить','поехать','заехать','забрать','купить','получить','взять',
    'отнести','занести','измерить','замерить','примерить','позвонить','написать','отправить',
    'прибить','повесить','помыть','убрать','сдать','распечатать','проверить','сделать',
    'доделать','подготовить','подготовиться','найти','заказать','записаться','оплатить',
    'вернуть','отдать','загрузить','скачать','прочитать','выучить','собрать','разобрать',
    'выбрать','составить','создать','оформить','организовать','спланировать','починить',
    'постирать','пропылесосить','протереть','вынести','согласовать','изучить'
  ];
  ...
  if(hits.length >= 2){
    ...
    for(let i=0;i<hits.length;i++){
      const text=clean.slice(hits[i].index, hits[i+1]?.index ?? clean.length)
        .replace(/^[,;.!?\s]+|[,;.!?\s]+$/g,'')
        .replace(/\s+(?:и|затем|потом|после этого)\s*$/i,'').trim();
      if(text) actions.push({text, verb:hits[i].verb});
    }

    // Путь до места + получение вещи обычно являются одним шагом.
    const merged=[];
    for(let i=0;i<actions.length;i++){
      const cur=actions[i], next=actions[i+1];
      if(next && /^(?:дойти|пойти|сходить|поехать|заехать)$/.test(cur.verb) &&
          /^(?:забрать|получить|купить|взять)$/.test(next.verb)){
    ...
    let object='';
    const objMatch=clean.match(/(?:забрать|купить|получить|взять|найти|заказать)\s+([а-яёa-z0-9-]+(?:\s+[а-яёa-z0-9-]+)?)(?=\s*(?:,|;|\.|$|\s+(?:и\s+)?(?:измерить|замерить|примерить|отнести|занести|вернуть|отдать)))/i);
    if(objMatch) object=objMatch[1].trim();
    if(!object){
      const simple=clean.match(/(?:забрать|купить|получить|взять|найти|заказать)\s+([а-яёa-z0-9-]+)/i);
      if(simple) object=simple[1].trim();
    }

    const result=actions.map(({text})=>{
  ...
  const tail=(re,fallback)=>{
    const m=clean.match(re);
    return (m && m[1] ? m[1].trim().replace(/[.!?]+$/,'') : fallback);
  };

  // Крупные задачи: локальные шаблоны дают полезные шаги даже когда в исходной
  // фразе только один глагол. Никакой внешний ИИ и интернет не требуются.
  if(/(?:убрать|убраться|генеральн.*уборк|навести порядок).*(?:дом|квартир|комнат)|(?:дом|квартир|комнат).*(?:убрать|уборк|порядок)/i.test(t)){
    return ['Собрать вещи и убрать всё с поверхностей','Протереть пыль и поверхности','Пропылесосить или подмести пол','Помыть пол','Убрать кухню и ванную'].slice(0,6);
  }
  if(/(?:подготовиться|подготовка).*(?:экзамен|зач[её]т|контрольн|тест)/i.test(t)){
    const subject=tail(/(?:к|на)\s+(?:экзамену|экзамен|зач[её]ту|зач[её]т|контрольной|тесту)\s*(?:по\s+)?(.+)$/i,'предмету');
    return [`Собрать материалы по ${subject}`,`Составить список тем по ${subject}`,`Повторить основные темы по ${subject}`,`Решить практические задания по ${subject}`,'Проверить темы, которые нужно повторить ещё раз'];
  }
  if(/(?:сделать|подготовить|доделать|создать).*(?:проект|курсов|отч[её]т|презентац)/i.test(t)){
    const thing=tail(/(?:сделать|подготовить|доделать|создать)\s+(.+)$/i,'работу');
    return [`Определить требования к задаче «${thing}»`,`Собрать материалы для «${thing}»`,`Составить структуру «${thing}»`,`Сделать основную часть «${thing}»`,`Проверить и исправить «${thing}»`];
  }
  if(/(?:подготовиться|собраться|подготовка).*(?:поездк|путешеств|отпуск)|(?:поездк|путешеств|отпуск).*(?:подготов|собра)/i.test(t)){
    return ['Проверить даты и маршрут поездки','Проверить документы и бронирования','Составить список вещей','Собрать необходимые вещи','Проверить всё перед выходом'];
  }
  if(/(?:организовать|подготовить|устроить).*(?:день рождения|праздник|вечеринк|мероприят)/i.test(t)){
    return ['Определить дату, место и бюджет','Составить список гостей','Продумать еду и напитки','Подготовить покупки и необходимое','Подтвердить детали перед мероприятием'];
  }
  if(/(?:переезд|переехать|переезжать)/i.test(t)){
    return ['Составить список вещей для переезда','Разобрать и упаковать вещи','Подготовить документы и ценности отдельно','Организовать перевозку','Проверить старое и новое помещение перед переездом'];
  }

  // Универсальный безопасный fallback для явно крупных задач.
  if(/^(?:подготовить|подготовиться|организовать|спланировать|сделать|создать|разобрать|изучить)\b/i.test(t) && clean.length >= 18){
    return [`Уточнить результат задачи: ${clean}`,`Собрать всё необходимое для задачи: ${clean}`,`Сделать основную часть задачи: ${clean}`,`Проверить результат задачи: ${clean}`].map(cap);
  }
